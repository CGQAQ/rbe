  ...
  workers: [
    {
      local: {
        ...
        // Chromium clang actions ship ~15k-file input trees (SDK, sysroot,
        // toolchain). Without this cache the worker re-materializes the
        // whole tree from the CAS for EVERY action and spends all its time
        // in download_to_directory instead of clang. Subtree caching reuses
        // byte-identical subtrees (the SDK barely changes) via hardlinks.
        directory_cache: {
          max_entries: 20000,
          max_size_bytes: "20gb",
